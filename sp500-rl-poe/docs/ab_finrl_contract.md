...
WRDS. Dro`p the two files below into data/raw/` and the rest of `sp500rl`
...
- `universe.rule: ab_finrl` — the 10 names selected in AB_finRL
(`AAPL, MSFT, JNJ, JPM, XOM, PG, HD, UNH, CAT, DIS`). Resolution order:
  sidecar CSV (`data/raw/ab_finrl_tickers.csv` when present),
  a `selected` / `sandbox` flag on `wrds_tickers`, then
  `universe.ab_finrl_tickers` in YAML / `configs/ab_finrl_tickers.csv`.
- `sandbox` / `top_n` still work on the canonical frame.

## What this repo does **not** do

No `wrds` Python package, no `db.describe_table`, no live CRSP pull. That
stays in AB_finRL. Only the two CSVs (and optionally the 10-name ticker list)
cross the boundary.

## 10-name sandbox (`ab_finrl`)

AB_finRL selected these 10 liquid names across sectors. They live in
`[configs/ab_finrl_tickers.csv](../configs/ab_finrl_tickers.csv)` and in
`universe.ab_finrl_tickers`. `--universe ab_finrl` filters the panel to this
set. If `wrds_tickers` has a `selected` (or `sandbox` / `in_portfolio`) column,
those flagged rows win over the YAML list.